...
monsterDictionary = ["Monster1", "Monster2",
                     "AlexMonster", "BenMonster", "LeoMonster", "MonsterBig"]
towerDictionary = {"Arrow Shooter": "ArrowShooterTower", "Bullet Shooter": "BulletShooterTower",
                   "Tack Tower": "TackTower", "Power Tower": "PowerTower"}
towerCost = {"Arrow Shooter": 150, "Bullet Shooter": 150,
             "Tack Tower": 150, "Power Tower": 200}
...
class Infoboard:
    def __init__(self, game):
        self.canvas = Canvas(master=game.frame, width=162,
                             height=174, bg="gray", highlightthickness=0)
        ...
        if issubclass(displayTower.__class__, TargetingTower):
            ...
            if displayTower.upgradeCost:
                ...
                self.canvas.create_text(120, 157, text="Upgrade: " + str(
                    displayTower.upgradeCost), font=("times", 12), fill="light green", anchor=CENTER)

            self.canvas.create_text(28, 146, text="Sell", font=(
                "times", 22), fill="light green", anchor=NW)
    ...
    def displayGeneric(self):
        ...
        if selectedTower == "<None>":
            ...
            self.text = selectedTower + " cost: " + \
                str(towerCost[selectedTower])
            self.towerImage = ImageTk.PhotoImage(Image.open(
...
class Displayboard:
    def __init__(self, game):
        self.canvas = Canvas(master=game.frame, width=600,
...
class Towerbox:
    def __init__(self, game):
        self.game = game
        self.box = Listbox(master=game.frame, selectmode="SINGLE", font=(
...
class ArrowShooterTower(TargetingTower):
    ...
    def shoot(self):
        ...
        projectiles.append(AngledProjectile(
...
class BulletShooterTower(TargetingTower):
    ...
    def shoot(self):
        projectiles.append(TrackingBullet(
